...
# Get analysis history, with optional limit input
@app.route(route="GetAnalysisHistory")
@app.cosmos_db_input(
    arg_name="inputDocument",
    database_name="cst8917lab1cosmosdb",
    container_name="analysis_results",
    connection="DATABASE_CONNECTION_STRING",
    sql_query="SELECT * FROM c ORDER BY c.metadata.analyzedAt DESC"
)
def GetAnalysisHistory(req: func.HttpRequest, inputDocument: func.DocumentList) -> func.HttpResponse:
    """
    This function retrieves the history of text analyses from Cosmos DB.
    Parameters:
        req: The incoming HTTP request (may contain optional limit parameter)
        inputDocument: The input binding to read analysis results from Cosmos DB
    Returns:
        func.HttpResponse: JSON response with analysis history
    """
    
    logging.info('Get Analysis History API was called!')

    # Parse optional ?limit=5
    ...
    # Convert DocumentList to list of dicts
    history = [doc.to_dict() for doc in inputDocument]
...
@app.cosmos_db_output(arg_name="outputDocument", database_name="cst8917lab1cosmosdb", 
    container_name="analysis_results", connection="DATABASE_CONNECTION_STRING")
def TextAnalyzer(req: func.HttpRequest, outputDocument: func.Out[func.Document]) -> func.HttpResponse:
    ...
    Parameters:
        ...
        outputDocument: The output binding to write the analysis result to Cosmos DB
    Returns:
    ...
    if text:
        ...
        response_data = {
            "id": str(uuid.uuid4()),  # Generate a unique ID for this analysis
            "analysis": {
            ...
            "originalText": text  # Include the original text in the response
        }
